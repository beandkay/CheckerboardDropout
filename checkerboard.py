...
def checkerboard(x: torch.Tensor, drop_prob: float = 0.1, drop_size: int = 1, drop_shape: str = 'square', drop_mode: str = 'ver'):
    B, C, H, W = x.shape
    total_size = W * H
    ...
    x = x.where(block_mask_indexes.ge(drop_prob), x * mask.to(dtype=x.dtype))

    return x


class Checkerboard(nn.Module):
    def __init__(self, drop_prob, drop_size, drop_shape, drop_mode):
    ...
    def forward(self, x):
        ...
        if not self.training or self.drop_prob == 0.:
            ...
            return checkerboard(
                x, self.drop_prob, self.drop_size, self.drop_shape, self.drop_mode)
